                      ...
                      {model && (
                        <Button asChild size="sm" variant="secondary" className="h-7 px-2 text-[10px]">
                          <Link to="/studio" search={{ model: l.modelId, look: l.id } as any}>
                            <Wand2 className="h-3 w-3 mr-1" /> Restyle
                          </Link>
                        </Button>
                      )}
                      <button onClick={() => removeLook(l.id)} className="opacity-60 hover:opacity-100 p-1"><Trash2 className="h-3.5 w-3.5 text-destructive" /></button>
                    </div>
                  </div>
                </div>
              </div>
